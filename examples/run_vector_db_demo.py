...
def main() -> int:
    ap = argparse.ArgumentParser(description="Run HUF over exported vector DB retrieval results.")
    ...
    ap.add_argument("--tau-global", type=float, default=0.01, help="Global exclusion threshold (mass frame).")
    ...
    ap.add_argument("--nonneg-mode", type=str, default="clip", choices=["clip", "shift"], help="How to handle negative scores.")
    ...
    cfg = VectorDBAdapterConfig(
        ...
        top_k=args.top_k,
        trace_include_fields=["source", "cluster", "collection"],
    )
